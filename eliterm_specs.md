...
- **技術スタック**:
  ...
  - ※ システムトレイから明示的に `Quit`（完全終了）が選択された場合のみ、プロセスツリー全体およびバックグラウンドに残存してインストールフォルダの削除を阻害する `epmd.exe` を確実に自動キルし、クリーンに終了する仕組みを備えている。
...
- セキュリティ・ブラウザ制限（Secure Context / ユーザー操作必須の制約）を完全に回避するため、Eliterm ではクリップボードの読み書きを主に **Elixir (wxWidgets) バックエンド経由** で行います。
- **アーキテクチャ詳細:**
  ...
  - OS ネイティブでメニュー（`wxMenuBar`）が `Cmd+C` などをキャッチした場合、一時的に OS のクリップボードがロックされる競合状態（Race Condition）を防ぐため、`Eliterm.Clipboard` 内部に最大 500ms (10回) のリトライ機構を搭載しています。
  - **フロントエンドの補完:** `app.js` 側のキーボードハンドラー（`keydown` イベント）でも、`navigator.clipboard` が利用可能であれば並行して書き込み・読み込みを行い、さらにフォールバックとして Elixir バックエンドと通信（`clipboard_copy` / `clipboard_paste` イベント）する冗長設計を採用しています。
  - **ペースト時のエスケープ:** ペースト時は生の文字列をバックエンドへ送るのではなく、`xterm.js` の `this.term.paste(text)` API を経由させます。これにより Bracketed Paste Mode が有効なシェル（zsh等）に対して、適切に `\x1b[200~` などのエスケープシーケンスが付与され、シンタックスハイライトによる不要な背景色（paste highlight 等）の描画を防ぎます。
