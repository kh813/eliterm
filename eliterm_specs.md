...
- メニューバーはOSの標準的なアプリケーションのレイアウトに準拠する。
  ...
  - **Cluster**: クラスタ情報の表示（ノード名とCookie）、クラスタの初期化（`cluster init`）、接続先を指定してのクラスタ参加（`cluster join`）、クラスタからの離脱（`cluster leave`）、ノード名接頭辞の変更（`cluster rename`）、およびクラスタのリセット（`cluster reset`）を、GUI のネイティブダイアログ（`wxMessageDialog` および `wxTextEntryDialog`）を用いて操作できる項目を配置する。
...
eliterm cluster init [<prefix>]
  # クラスタを初期化（ノード接頭辞を任意指定可能。すでに初期化されている[Cookieファイルが存在する]場合はエラーとなり実行できません。再初期化・再参加を行うには事前に `cluster reset` を実行する必要があります。デーモン非起動時はローカルに直接クッキーを作成）

eliterm cluster join <node> [--cookie <cookie>] [--role <role>]
  # 既存クラスタに参加（すでに初期化されている[Cookieファイルが存在する]場合はエラーとなり実行できません。再参加を行うには事前に `cluster reset` を実行する必要があります。--cookie 指定時はクッキーを設定ファイルに保存して動的適用。--role には secondary [管理者] または member [一般、デフォルト] を指定します）
...
eliterm cluster reset
  # クラスタ接続情報および設定のリセット（Erlang分散ノードを停止・離脱し、Cookieファイルを削除して未初期化状態に戻します）
...
```bash
# プライマリノード（最初の1台、オプションでプレフィックスを指定）
eliterm cluster init primarycluster

# 追加ノード
# (注意: すでに初期化済みの場合は、事前に `eliterm cluster reset` を実行しておく必要があります)
...
- **プライマリノード (管理ノード)**:
  - `eliterm cluster init` を実行してクラスタを構築したノード。1クラスタに1ノードのみ存在します。
  - クラスタ全体の管理権限を持ちます。ノードの改名 (`cluster rename`) や、クラスタのリセット (`cluster reset`) を実行可能です。
  - プライマリノードが `cluster reset` を実行した際、他のノードに対して「連鎖的離脱（Cascade Leave）」を指示できます。
- **セカンダリノード (管理者ノード)**:
  - `eliterm cluster join <node> --role secondary` を実行して参加したノード。複数存在して構いません。
  - クラスタ自体の設定変更や初期化（ノードの改名 `cluster rename` やクラスタのリセット `cluster reset`）の権限は持ちません。クラスタ関連で実行可能な操作は、情報の確認 (`cluster info`) およびクラスタからの安全な離脱 (`cluster leave`) のみです。 (※セッション管理など、将来的なクラスタ以外の管理者向け操作のためにロール枠を設けています)
- **メンバーノード (一般ノード)**:
  - デフォルトの `eliterm cluster join <node>` （`--role member`）を実行して参加したノード。
  - 管理権限を必要とする操作 (`cluster rename` や `cluster reset`) は実行できません。これらのコマンドを実行するとエラーになります。
  - クラスタから離脱する場合は、`eliterm cluster leave` を使用します。

#### 連鎖的離脱 (Cascade Leave)
- プライマリノードで `eliterm cluster reset` を実行した際、他に接続されているセカンダリノードやメンバーノードが存在する場合は、それらすべてのノードに対して連鎖的に離脱（`leave`）シグナルが送信されます。
- これにより、プライマリノードのリセットに合わせて、接続されていた他ノードも自動的にクラスタから安全に離脱し、Cookie や設定がリセットされ、個別の未初期化状態へと戻ります。
- 実行前に、他に接続中のノードがある場合はその一覧を提示し、強制離脱を伴うリセットである旨を伝える強力な警告プロンプトが表示されます。
...
### 12.5 招待トークンを用いたクラスタ参加フロー (Cluster Join via Invite Token)

初期化されたクラスタに対して新規ノードを参加させる際、利便性と堅牢なセキュリティを両立させるため、プライマリ（親）ノードの管理者による「ワンタイム招待トークン（認証コード）」を用いた参加フローを提供します。
プライマリ側は常時接続を受け付けるわけではなく、明示的に招待コマンドを実行した時のみ、期間限定で参加窓口を開きます。他ノードが正しいトークンを入力して参加完了すると、窓口は自動的に閉じられます。
...
sequenceDiagram
    autonumber
    actor Primary as プライマリ管理者
    participant P as プライマリノード (Primary)
    participant M as 参加ノード (Member)
    actor Member as 参加ノード管理者

    Primary->>P: eliterm cluster invite (または GUI から招待ボタンを押す)
    Note over P: ワンタイムトークン "582-901" を生成<br>5分間の参加受付状態（Invite Mode）を開始
    P-->>Primary: トークン表示: "582-901"
    Primary-->>Member: トークンを伝達 (口頭、チャットなど)

    Member->>M: eliterm cluster join <PrimaryNode> --token 582-901
    Note over M: 一時非対称鍵ペアを生成 (Public/Private Key)
    M->>P: 参加申請 (HTTP/API)<br>・ノード名<br>・一時公開鍵<br>・トークン "582-901"
    
    Note over P: トークンと有効期限を検証
    alt トークンが有効な場合
        Note over P: 自身の Cookie を参加ノードの公開鍵で暗号化<br>招待状態を即座に破棄（受付終了）
        P->>M: 暗号化された Cookie を返却
        Note over M: 自身の秘密鍵で Cookie を復号して保存
        M->>P: 正式接続 (Erlang 分散 & Node.connect)
        M-->>Member: 接続成功・自動参加完了
    else トークンが無効または期限切れの場合
        P->>M: エラー応答 (Invalid/Expired Token)
        M-->>Member: 接続失敗エラー
    end
```
...
##### 1. プライマリでの招待トークン発行 (招待モード開始)
* **招待コマンド**:
  ```bash
  bin/eliterm cluster invite
  ```
* **動作詳細**:
  * コマンド（または GUI の `Invite Node...` メニュー）を実行すると、ランダムな一時トークン（例：`582-901`）を生成します。
  * デーモンのメモリ上に、このトークンと有効期限（生成から5分間）を記録し、招待受付状態（Invite Mode）に入ります。
  * CLI 上では、招待トークンを表示するとともに、対話的な接続待ち受け画面になります。
  * ユーザーが待機中に `ESC` キー（アスキーコード 27）または `Ctrl+C` を押すと、即座に接続待ち受けを強制終了し、発行されたトークンを無効化（招待受付を即座に終了）します。
  * 出力例:
    ```
    Generated invite token: 582-901
    Waiting for a node to join... (Expires in 5 minutes)
    Press ESC or Ctrl+C to cancel.

    # キャンセル実行時:
    Invite canceled. Token invalidated.
    ```
  * GUI から実行した場合は、招待トークンと残りカウントダウン（5分間）を表示するモーダルダイアログ内に「Cancel」ボタンが配置され、それをクリックすることで同様に強制終了できます。

##### 2. 参加ノードでのジョインリクエスト送信
* **参加コマンド**:
  ```bash
  bin/eliterm cluster join <PrimaryNode> --token <Token> [--role <role>]
  ```
  * 実行例: `bin/eliterm cluster join my-cluster@primary-host --token 582-901 --role member`
* **動作詳細**:
  * 参加ノード側で一時的な公開鍵・秘密鍵のペア（非対称鍵）を生成します。
  * プライマリノードの HTTP(S) ポートに対して、トークン付きの参加リクエスト（JSON POST）を送信します。
  * 送信データ：
    ```json
    {
      "token": "582-901",
      "node_name": "member1@member-host",
      "role": "member",
      "public_key": "PEM-encoded-temporary-public-key"
    }
    ```

##### 3. トークン検証と安全な接続確立
* **トークン検証と使い捨て処理 (One-Time)**:
  * プライマリノードは、送られてきた `token` が現在保持している有効な未期限のトークンと一致するかを検証します。
  * トークンが一致した場合、プライマリノードは**即座にその招待トークンおよび受付状態を破棄**します。これにより、同じトークンを使用した複数ノードの参加や、横取り攻撃（リプレイ攻撃）を完全に防止します。
* **Cookie の暗号化送信**:
  * 検証が成功すると、プライマリノードは現在クラスタで共有している「Cookie（クッキー）」を、参加ノードから送られてきた `public_key` を用いて暗号化して返却します。
  * 参加ノードは自身の秘密鍵でこのクッキーを復号し、設定ファイル `~/.config/eliterm/cookie` に保存します。
  * 取得した Cookie を用いて `Node.set_cookie/1` を設定し、`Node.connect/1` を実行して Erlang 分散を接続して自動的に参加が完了します。
* **検証失敗時**:
  * トークンが不一致、またはすでに有効期限（5分）が切れている場合、エラーを返します。参加ノード側はクッキーを取得できず、エラーを出力して終了します。
