...
- メニューバーはOSの標準的なアプリケーションのレイアウトに準拠する。
  ...
  - **Cluster**: クラスタ情報の表示（ノード名とCookie）、クラスタの初期化（`cluster init`）、接続先を指定してのクラスタ参加（`cluster join`）、クラスタからの離脱（`cluster leave`）、ノード名接頭辞の変更（`cluster rename`）を、GUI のネイティブダイアログ（`wxMessageDialog` および `wxTextEntryDialog`）を用いて操作できる項目を配置する。
