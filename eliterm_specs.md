...
|---|---|---|
| **macOS** | `NSWorkspace.willSleepNotification` と `IOPMAssertion` | **Swift**<br>Elixir の開発環境（Xcode CLT）に標準搭載されている `swiftc` を用い、インストール時に50行程度の小さなヘルパーバイナリをコンパイルする。依存ライブラリを増やさずネイティブ API を叩ける。 |
| **Linux** | `systemd-inhibit` (DBus) | **OS標準 CLI**<br>C や Rust を用いず、Elixir から `systemd-inhibit --mode=delay` コマンドを実行するだけでスリープ遅延を実現する。 |
| **Windows** | `SystemEvents.PowerModeChanged` (C# / PowerShell) | **PowerShell / C# (未実装)**<br>`.NET` の `SystemEvents.PowerModeChanged` を監視する軽量なスクリプトを記述し、Elixir からプロセスとして起動して検知する設計。スリープ遅延には `PowerCreateRequest` API が必要となる。 |
