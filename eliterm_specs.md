...
- **対応OS**: macOS, Windows（Linuxは非サポート）
  - ※ Windows環境でWSL等のローカルシェルにフォールバックして起動する場合、隔離された `home/` ではなくネイティブなLinuxホームディレクトリ (`~/`) から開始されるよう初期パスを調整し、確実なインタラクティブシェル起動およびタブ補完有効化のため `bash.exe` ではなく `wsl.exe` コマンドを使用する。
- **設定永続化の仕組み**: 
...
- **技術スタック**:
  ...
  - `xterm.js`: LiveView 内の WebView に描画されるターミナルエミュレータコンポーネント（※ Windows版での表示乱れや過剰なサブピクセルアンチエイリアスを防ぐため、バックグラウンドを透過設定にしてOS標準のアンチエイリアス設定にフォールバックさせる）

CLI 実行時と同じ `Eliterm.Application` のスーパーバイザーツリーを起動しつつ、CLI の代わりに GUI ウィンドウを表示する構成とする。
...
- メニューバーはOSの標準的なアプリケーションのレイアウトに準拠する。
  ...
  - **View**: メジャーなカラースキーム（Monokai, Solarized, Dracula, GitHub, Catppuccin など）のプリセットをワンクリックで選択・リアルタイム反映できる `Color Scheme` サブメニューや、ターミナルのフォントファミリー（Consolas, Cascadia Code, Menlo, Monaco, Fira Code など）を動的に切り替えられる `Font` サブメニューを配置する。指定したフォントがシステムに未インストールの場合でも表示が崩れないよう、必ず `, monospace` などのフォールバックをCSS側（xterm.js設定）で適用する。特にWindows環境での視認性向上のため、デフォルトのフォントスタックは `Consolas` を最優先とする。
