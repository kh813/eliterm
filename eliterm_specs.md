...
- メニューバーはOSの標準的なアプリケーションのレイアウトに準拠する。
  ...
  - **View**: メジャーなカラースキーム（Monokai, Solarized, Dracula, GitHub, Catppuccin など）のプリセットをワンクリックで選択・リアルタイム反映できる `Color Scheme` サブメニューや、ターミナルのフォントファミリー（Menlo, Monaco, Fira Code など）を動的に切り替えられる `Font` サブメニューを配置する。指定したフォントがシステムに未インストールの場合でも表示が崩れないよう、必ず `, monospace` などのフォールバックをCSS側（xterm.js設定）で適用する。
