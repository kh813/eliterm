...
これらの `eliterm cluster` 関連のコマンドは、ホストOS（Mac/Windows）のターミナルから実行できるだけでなく、**Eliterm のターミナルセッション内（ゲストコンテナ内）から直接実行することも可能**です。コンテナ内から `eliterm` コマンドを実行した場合は、自動的に Unix ドメインソケット（`~/.eliterm-cli.sock`）を経由してホスト上のデーモンに処理が中継され、確認プロンプトなども含めてそのままターミナル上で対話的に動作します。
