...
Eliterm はバックグラウンドで動作する Elixir プロセス（デーモン）として動作します。
事前に [Elixir (v1.15以上)](https://elixir-lang.org/install.html) をインストールしておいてください。

### macOS / Linux

**Git がある場合:**
```bash
git clone https://github.com/kh813/eliterm.git
cd eliterm
./install.sh
```

**Git がない場合 (curl & unzip):**
```bash
...
> **注意**: `install.sh` / `install.ps1` を実行すると、必要なライブラリのダウンロードとコンパイルが行われ、`bin/eliterm` という実行ファイルが生成されます。必要に応じて `bin/` ディレクトリに PATH を通してください。
