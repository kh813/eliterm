...
**💡 ゲスト環境（コンテナ内）からの実行について**
これらの `eliterm cluster` 関連のコマンドは、ホストOS（Mac/Windows）のターミナルから実行できるだけでなく、**Eliterm のターミナルセッション内（ゲストコンテナ内）から直接実行することも可能**です。コンテナ内から `admin` コマンドを実行した場合は、自動的に一時ポートファイル（`~/.eliterm-cli.port`）から情報を読み取って TCP 接続を経由してホスト上のデーモンに処理が中継され、確認プロンプトなども含めてそのままターミナル上で対話的に動作します。
