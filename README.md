...
1. **クラスタを初期化します**
   ```bash
   bin/eliterm cluster init [クラスター名]
   ```
   任意のクラスター名（ノード名接頭辞、例: `my-cluster`）を指定して初期化します。指定しない場合はデフォルトの `eliterm` となります。
   これにより、クラスタ接続用のランダムな Cookie が生成され、ノードが起動します。

   > **注意**: 一度初期化（Cookie ファイルが作成）されたノードは、安全のため再度 `cluster init` や `cluster join` を直接実行することはできません。再初期化・再構築を行いたい場合は、事前に `bin/eliterm cluster reset` を実行して初期化状態に戻す必要があります。
...
2. **接続情報を確認します**
   ...
   出力例：
   ```
   Node: my-cluster@server-host
   Cookie: AVERYSECRETRANDOMCOOKIEVALUE...
   ```
   ここに表示される `Node` 名（例: `my-cluster@server-host`）と `Cookie` の値を、2台目の接続の際に使用します。
...
```bash
bin/eliterm cluster join <1台目のNode名> --cookie <1台目のCookie値> [--role <role>]
```

**実行例:**
```bash
bin/eliterm cluster join my-cluster@server-host --cookie AVERYSECRETRANDOMCOOKIEVALUE... --role member
```
`--role` オプションには `secondary`（ノードの改名やリセットが可能な管理者ロール、複数指定可）または `member`（一般ユーザーロール、デフォルト）を指定できます。
これにより、2台目のPCにも同じ Cookie が永続化され、自動的にクラスタへ参加します。

### (オプション) ノード名の変更 (プライマリノードのみ)
プライマリノードは、以下のコマンドで自身のノード接頭辞（クラスター名）を変更できます。
...
- **クラスタからの安全な離脱 (セカンダリ / メンバーノード向け)**:
  ```bash
  bin/eliterm cluster leave
  ```
  一般ノードがクラスタから安全に離脱し、Cookie や設定をリセットして初期状態に戻します。

- **クラスタの解散と設定リセット (プライマリノード向け)**:
  ```bash
  bin/eliterm cluster reset
  ```
  Erlang 分散ノードを停止・離脱し、Cookie ファイルを削除して完全な未初期化状態に戻します。
  プライマリノード（親）でリセットを実行した場合、クラスタに接続されている他のすべての一般・管理者ノードに対しても連鎖的離脱（Cascade Leave）のシグナルが送られ、クラスタ全体が安全に解散・初期化されます。
