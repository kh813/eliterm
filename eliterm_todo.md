...
- [x] アプリケーションのシステムトレイ（常駐）化および `Quit` による明示的なプロセス完全終了の実装
- [x] ウィンドウの「×」ボタンや `exit` 実行時はアプリを終了せず、バックグラウンドに隠す挙動へ変更
- [x] WSL・ローカルシェルへのフォールバック起動ロジックの廃止
- [x] 起動時に Docker/Podman のインストール状態をチェックし、未インストールの場合はネイティブダイアログで警告して実行を停止する処理の実装
- [x] OS標準のターミナルフォントのみをデフォルトのフォントメニューに表示するようOS判定ロジックを実装（Mac: Menlo, Monaco 等、Windows: Consolas）
- [x] 「Scan & update font list」メニューを追加し、ローカルにインストールされている既知の開発者向けフォントをスキャンして設定 (TOML) に永続化する機能を実装

## Phase 9: 動的クラスタリングの改善とCLI強化（v0.1.25）

- [x] デーモンの「自動分散起動」の追加：起動時にクラスタ初期化（クッキー生成）と Erlang 分散ノードの自動起動を実装
- [x] `eliterm cluster init` のローカルフォールバック実装：デーモン非起動時は CLI から直接クッキーファイルを生成
- [x] `eliterm cluster join <node> --cookie <cookie>` の実装：クッキーを指定してクラスタへ動的に参加し、クッキーを永続化
...
## Phase 10: 招待トークンを用いたセキュア接続 (v0.1.26)

- [x] プライマリ側での一時的な「招待モード（Invite Mode）」および招待トークン（5分有効）管理機能の実装
- [x] 参加ノード側で一時的な非対称暗号鍵ペア（RSA 等）を生成し、トークンと共に HTTP POST で申請を送信する処理の実装
- [x] プライマリ側での HTTP 参加申請受け口（API エンドポイント）の Phoenix Endpoint/Router への実装
- [x] プライマリ側でのトークン検証（一致判定・期限チェック）と、成功時の即座のトークン無効化（使い捨て処理）の実装
- [x] 承認時のクッキー暗号化返却、参加ノード側での復号・保存、および Erlang 分散接続 of 自動確立の実装
- [x] CLI に `eliterm cluster invite` サブコマンドを追加し、トークンを表示して接続を対話的に待機する処理の実装
- [x] CLI の `invite` 実行中に `ESC` キーまたは `Ctrl+C` を押すことで、即座に接続待ちをキャンセルしてトークンを無効化するループ処理の実装
- [x] GUI メニューバーに `Invite Node...` 項目を追加し、カウントダウン表示と「Cancel」ボタンを備えたダイアログでのキャンセル処理を実装
- [x] 招待トークンを用いたクラスタジョイン、自動承認、キャンセル、および暗号化通信 of ユニットテストの追加・修正
  - [x] テスト時におけるシングルプロセスのファイル競合（Cookie削除と読み込みの競合）の修正
  - [x] 警告のクリーンアップ（`terminal_live_test.exs` での未使用アトリビュート警告等の解消）
  - [x] 未初期化状態（Cookieファイル未生成）で `cluster invite`/`token` を実行した際の警告・ガード処理の追加
  - [x] 複数 Docker コンテナによる E2E クラスター疎通テストスクリプト（`test_docker_cluster.sh`）の作成
