...
- Phase 7: GUI安定化・機能補強（v0.1.16）の計画を開始し、以下の改善を実装・ドキュメント化しました。
  ...
  - Windows環境にて、Unix Domain Socket (`AF_UNIX`) がサポートされていないため `:eafnosupport` エラーによりターミナルセッション (PTY) がクラッシュする問題を修正。Windows環境でもGUIアプリ自体は正常に起動し、ターミナルセッションが開始できるように改善 (v0.1.17)。
  - Windows環境等において、ウィンドウを閉じた際にErlang VMが終了せずバックグラウンドで残り続け、次回起動時にポート枯渇（eaddrinuse）を引き起こす問題を修正。`Eliterm.WindowWatcher` を導入し、ウィンドウが閉じられた（非表示になった）ことを検知して確実に `System.halt(0)` でアプリケーションを終了するように改善 (v0.1.17)。
  - Windows環境におけるメニューバーのキーボードショートカット（Cmd vs Ctrl）の違いによりメニューが正常に構築されない・表示が崩れる問題を修正し、動的に `Ctrl` を使用するように対応 (v0.1.18)。
  - Windows環境でのターミナル描画フォントのデフォルトスタックに `Cascadia Code` および `Consolas` を優先追加し、メニューバーから直接 `Cascadia Code` を選択できるように改善 (v0.1.18)。
  - Windows上での `ExPTY` 初期シェルとしてWSL環境が起動する挙動は、システムデフォルトの bash 解決仕様として正常であることを確認。
  - Windows環境でのターミナル描画フォントのデフォルトスタックにおいて、`Consolas` を最優先とするように変更 (v0.1.19)。
  - Windows/Mac等でコンテナフォールバック（ローカル実行）を使用する際、新規ターミナルの起動ディレクトリがElitermのセッション作業ディレクトリではなく、ネイティブなユーザーホームディレクトリ (`~/`) になるように修正。特にWSL上で `~/` (Linuxホーム) から開始されるよう挙動を改善 (v0.1.19)。
