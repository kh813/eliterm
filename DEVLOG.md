...
- Phase 7: GUI安定化・機能補強（v0.1.16）の計画を開始し、以下の改善を実装・ドキュメント化しました。
  ...
  - Windows環境におけるメニューバーのキーボードショートカット（Cmd vs Ctrl）の違いによりメニューが正常に構築されない・表示が崩れる問題を修正し、動的に `Ctrl` を使用するように対応。
  - Windows環境でのターミナル描画フォントのデフォルトスタックにおいて、`Consolas` を最優先とし、`Cascadia Code` をそれに続く優先順位として追加。メニューバーからもこれらを動的変更できるように実装。
  - Windows/Mac等でコンテナフォールバック（ローカル実行）を使用する際、新規ターミナルの起動ディレクトリが隔離環境 (`home/`) ではなく、ネイティブなユーザーホームディレクトリ (`~/`) になるように修正。特にWSL上で `~/` (Linuxホーム) から確実に開始されるよう挙動を改善。
  - `Eliterm.Config` モジュールを導入し、`eliterm.toml` ファイルを用いたGUI設定の永続化を一元管理する仕組みを実装。これにより、ウィンドウサイズや位置、フォントやカラースキームなどの状態が次回起動時にも復元されるように改善 (v0.1.18)。
